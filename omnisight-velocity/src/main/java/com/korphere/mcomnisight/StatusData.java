...
import com.korphere.mcomnisight.provider.VMemoryFromRuntimeProvider;
//import com.korphere.mcomnisight.provider.PlayerDataProvider;
import com.korphere.mcomnisight.provider.VStandardDataProvider;
import com.korphere.mcomnisight.provider.VOshiProvider;
...
public class StatusData {
    public static boolean oshiAvailable = false;

    public static MCOmniSightVelocity plugin;
    private static volatile JsonObject lastFullData = null;

    public static void init(MCOmniSightVelocity instance) {
        plugin = instance;
        try {
    ...
    public static @NotNull JsonObject collect() {
        ...
        JsonObject standardData = VStandardDataProvider.getStandardData();
        if (standardData != null) json.add("standard", standardData);

        JsonObject performance = new JsonObject();

        JsonObject memory = VMemoryFromRuntimeProvider.getMemoryFromRuntime();
        ...
        if (oshiAvailable) {
            json.add("host_status", VOshiProvider.getHostData());
        }
    ...
    public static void sendInitialFullData(org.java_websocket.WebSocket conn, boolean useGzip) {
        ...
        plugin.getServer().getScheduler().buildTask(plugin, () -> {
            try {
                ...
                plugin.getLogger().error("Initial data transmission error: {}", e.getMessage());
            }
        }).schedule();
    }

    public static void sendInitialFullDataTcp(OmniSightTcpServer.ClientHandler client, boolean useGzip) {
        JsonObject currentData = collect();
        ...
        plugin.getServer().getScheduler().buildTask(plugin, () -> {
            try {
                ...
                if (useGzip) {
                    ...
                    String jsonString = currentData + "\n";
                    client.sendBytes(jsonString.getBytes(StandardCharsets.UTF_8));
                }
            } catch (Exception e) {
                plugin.getLogger().error("TCP Initial data transmission error: {}", e.getMessage());
            }
        }).schedule();
    }
}