...
import com.korphere.mcomnisight.provider.*;
...
public class StatusData {
    ...
    public static MCOmniSightPaper plugin;
    private static volatile JsonObject lastFullData = null;

    public static void init(MCOmniSightPaper instance) {
        plugin = instance;
        try {
    ...
    public static void sendInitialFullDataTcp(OmniSightTcpServer.ClientHandler client, boolean useGzip) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            try {
                ...
                plugin.getLogger().severe("TCP Initial data transmission error: " + e.getMessage());
            }
        });
    }
}
